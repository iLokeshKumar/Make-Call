...
type KPI = { pending: number; approved_today: number; rejected_today: number };
...
export default function PurchaseIndentsTab({ sessionTimeout: _s }: { sessionTimeout?: () => void }) {
  ...
  const { data: kpis } = useQuery({
    ...
    queryFn: () => apiGet<KPI>(`${CRM_BASE}/purchase/kpis`),
  });
  ...
  const reject = useMutation({
    mutationFn: ({ id, reason }: { id: number; reason: string }) =>
      apiPost(`${CRM_BASE}/purchase/indents/${id}/reject`, { reason }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["purchase-indents"] }); toast.success("Indent rejected"); },
  });

  const bulkApprove = useMutation({
    mutationFn: (ids: number[]) => apiPost(`${CRM_BASE}/purchase/indents/bulk-approve`, { indent_ids: ids }),
  ...
  const pendingA2 = indents.filter(i => i.autonomy_level === "A2" && i.status === "pending");
  const allSelected = pendingA2.length > 0 && pendingA2.every(i => selected.has(i.id));
      ...
      {kpis && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: "Approved Today", value: kpis.approved_today },
            { label: "Rejected Today", value: kpis.rejected_today },
          ].map(k => (
          ...
          <thead className="bg-slate-50 dark:bg-slate-800/60 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              <th className="px-4 py-3 w-8">
                <input type="checkbox" checked={allSelected} onChange={() => allSelected ? setSelected(new Set()) : setSelected(new Set(pendingA2.map(i => i.id)))} className="rounded" />
              </th>
            ...
            {isLoading ? (
              ...
              <tr key={i.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                <td className="px-4 py-3">
                  {i.autonomy_level === "A2" && i.status === "pending" && (
                    <input type="checkbox" checked={selected.has(i.id)}
                      onChange={() => setSelected(prev => { const n = new Set(prev); n.has(i.id) ? n.delete(i.id) : n.add(i.id); return n; })}
                      className="rounded" />
                  )}
                </td>
                <td className="px-4 py-3 font-mono text-xs text-slate-500">#{i.id}</td>
                <td className="px-4 py-3 text-slate-800 dark:text-slate-200">{i.requested_by_name ?? "—"}</td>
