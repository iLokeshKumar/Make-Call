"""
calcom_connector.py - Cal.com OAuth 2.1 + DCR flow for Rio CRM.

Cal.com's MCP server uses OAuth 2.1 with Dynamic Client Registration (RFC 7591)
instead of pre-registered client credentials. Rio self-registers as an OAuth
client at first connect, then runs a standard auth-code + PKCE popup flow.

Setup (one-time per Rio instance):
  Set in .env:
    CAL_COM_REDIRECT_URI=http://localhost:6060/crm/calcom/callback
    FRONTEND_URL=http://localhost:3006

Auth server (discovered from mcp.cal.com):
...
Flow:
  GET    /crm/calcom/auth-url   → DCR (first time) + PKCE auth URL for popup
  GET    /crm/calcom/callback   → code exchange, store tokens, redirect to UI
  GET    /crm/calcom/status     → connected?
  DELETE /crm/calcom/disconnect → purge tokens + MCP server row
  POST   /crm/calcom/refresh    → manual token refresh
"""
...
router = APIRouter(prefix="/crm/calcom", tags=["Cal.com"])

_PROVIDER   = "calcom"
_MCP_URL    = "https://mcp.cal.com/mcp"
_MCP_BASE   = "https://mcp.cal.com"
...
def get_company_calcom_token(session: Session, company_id: int) -> Optional[str]:
    """Return the stored Cal.com access token (used by capability router)."""
...
async def get_auth_url(
    ...
    if not client_id:
        ...
        logger.info("[calcom] DCR registered client_id for company %s", current_user.company_id)
...
async def calcom_callback(
    ...
    """
    Step 2: Cal.com redirects here after user approves.
    ...
    if error:
        logger.warning("[calcom] OAuth error from Cal.com: %s", error)
        return RedirectResponse(url=f"{frontend_base}/settings?section=mcp_connections&calcom=error")
    ...
    try:
        ...
        logger.error("[calcom] Token exchange failed: %s", exc.response.text)
        return RedirectResponse(url=f"{frontend_base}/settings?section=mcp_connections&calcom=error")
    except Exception as exc:
        logger.error("[calcom] Token exchange error: %s", exc)
        return RedirectResponse(url=f"{frontend_base}/settings?section=mcp_connections&calcom=error")

    access_token  = tokens.get("access_token")
    refresh_token = tokens.get("refresh_token")
    if not access_token:
        logger.error("[calcom] No access_token in response: %s", tokens)
        return RedirectResponse(url=f"{frontend_base}/settings?section=mcp_connections&calcom=error")
    ...
    if tool_count <= 0:
        logger.error("[calcom] No tools discovered for company %s — connection not usable", company_id)
        return RedirectResponse(url=f"{frontend_base}/settings?section=mcp_connections&calcom=error")

    logger.info("[calcom] Company %s connected via OAuth (%d tools)", company_id, tool_count)
    return RedirectResponse(
        url=f"{frontend_base}/settings?section=mcp_connections&calcom=connected"
    )
...
async def refresh_token(
    ...
    if not rt:
        raise HTTPException(status_code=400, detail="No refresh token — reconnect Cal.com")
    client_id = _get(session, current_user.company_id, "dcr_client_id")
    if not client_id:
        raise HTTPException(status_code=400, detail="No DCR client_id — reconnect Cal.com")
    try:
    ...
    if not new_token:
        raise HTTPException(status_code=400, detail="Refresh failed — reconnect Cal.com")
...
def _redirect_uri(request: Request) -> str:
    return os.getenv(
        ...
        str(request.base_url).rstrip("/") + "/crm/calcom/callback",
    )
